document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Only create offcanvas instance if element exists
    const mainOffcanvas = dom.mainOffcanvasElement ? new bootstrap.Offcanvas(dom.mainOffcanvasElement) : null;
        ...
        svgElement.addEventListener('click', (event) => {
            ...
            if (target) {
                if (highlightedElement && highlightedElement.id === target.id) {
                    if (mainOffcanvas) mainOffcanvas.hide();
                    return;
                }
